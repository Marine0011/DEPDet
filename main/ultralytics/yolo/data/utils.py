        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                ...
                assert max_cls <= num_cls, \
                    f'Label class {max_cls} exceeds dataset_SSDD class count {num_cls}. ' \
...
def check_det_dataset(dataset, autodownload=True):
    """Download, check and/or unzip dataset_SSDD if not found locally."""
    ...
    # Resolve paths
    path = Path(extract_dir or data.get('path') or Path(data.get('yaml_file', '')).parent)  # dataset_SSDD root
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            name = clean_url(dataset)  # dataset_SSDD name with URL auth stripped
            m = f"\nDataset '{name}' images not found ⚠️, missing paths %s" % [str(x) for x in val if not x.exists()]
            if s and autodownload:
                LOGGER.warning(m)
            else:
                m += f"\nNote dataset_SSDD download directory is '{DATASETS_DIR}'. You can update this in '{SETTINGS_YAML}'"
...
def check_cls_dataset(dataset: str, split=''):
    """
    Check a classification dataset_SSDD such as Imagenet.

    This function takes a `dataset_SSDD` name as input and returns a dictionary containing information about the dataset_SSDD.
    If the dataset_SSDD is not found, it attempts to download the dataset_SSDD from the internet and save it locally.

    Args:
        dataset (str): Name of the dataset_SSDD.
        split (str, optional): Dataset split, either 'val', 'test', or ''. Defaults to ''.

    Returns:
        data (dict): A dictionary containing the following keys and values:
            'train': Path object for the directory containing the training set of the dataset_SSDD
            'val': Path object for the directory containing the validation set of the dataset_SSDD
            'test': Path object for the directory containing the test set of the dataset_SSDD
            'nc': Number of classes in the dataset_SSDD
            'names': List of class names in the dataset_SSDD
    """
...
class HUBDatasetStats():
    """
    Class for generating HUB dataset_SSDD JSON and `-hub` dataset_SSDD directory

    Arguments
        path:           Path to data.yaml or data.zip (with data.yaml inside data.zip)
        task:           Dataset task. Options are 'detect', 'segment', 'pose', 'classify'.
        autodownload:   Attempt to download dataset_SSDD if not found locally

    Usage
        from ultralytics.yolo.data.utils import HUBDatasetStats
        stats = HUBDatasetStats('/Users/glennjocher/Downloads/coco8.zip', task='detect')  # detect dataset_SSDD
        stats = HUBDatasetStats('/Users/glennjocher/Downloads/coco8-seg.zip', task='segment')  # segment dataset_SSDD
        stats = HUBDatasetStats('/Users/glennjocher/Downloads/coco8-pose.zip', task='pose')  # pose dataset_SSDD
    ...
    def __init__(self, path='coco128.yaml', task='detect', autodownload=False):
        """Initialize class."""
        LOGGER.info(f'Starting HUB dataset_SSDD checks for {path}....')
    ...
    def _hub_ops(self, f):
        ...
        compress_one_image(f, self.im_dir / Path(f).name)  # save to dataset_SSDD-hub

    def get_json(self, save=False, verbose=False):
        """Return dataset_SSDD JSON for Ultralytics HUB."""
        ...
        def _round(labels):
            ...
            if self.task == 'detect':
                ...
                raise ValueError('Undefined dataset_SSDD task.')
...
def compress_one_image(f, f_new=None, max_dim=1920, quality=50):
    ...
    Usage:
        ...
        for f in Path('/Users/glennjocher/Downloads/dataset_SSDD').rglob('*.jpg'):
...
def delete_dsstore(path):
    ...
    Usage:
        ...
        delete_dsstore('/Users/glennjocher/Downloads/dataset_SSDD')
