...
"""
Dataloaders and dataset_SSDD utils
"""
...
def create_dataloader(path,
    ...
    if rect and shuffle:
        ...
        shuffle = False
    with torch_distributed_zero_first(rank):  # init dataset_SSDD *.cache only once if DDP
...
class _RepeatSampler:
    ...
    def __init__(self, sampler):
        """Sampler that repeats dataset_SSDD samples infinitely."""
...
class LoadImagesAndLabels(Dataset):
    ...
    cache_version = 0.6  # dataset_SSDD labels *.cache version
    ...
    def __init__(self,
        ...
        if min_items:
            ...
            LOGGER.info(f'{prefix}{n - len(include)}/{n} images filtered from dataset_SSDD')
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=''):
        ...
        for _ in range(n):
            ...
            b += im.nbytes * ratio ** 2
        mem_required = b * self.n / n  # GB required to cache dataset_SSDD into RAM
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        # Cache dataset_SSDD labels, check images and read shapes
        if path.exists():
    ...
    def __getitem__(self, index):
        """Get a sample and its corresponding label, filename and shape from the dataset_SSDD."""
    ...
    def load_image(self, i):
        """Loads 1 image from dataset_SSDD index 'i', returns (im, original hw, resized hw)."""
...
def extract_boxes(path=DATASETS_DIR / 'coco128'):  # from utils.dataloaders import *; extract_boxes()
    # Convert detection dataset_SSDD into classification dataset_SSDD, with one directory per class
...
def autosplit(path=DATASETS_DIR / 'coco128/images', weights=(0.9, 0.1, 0.0), annotated_only=False):
    """Autosplit a dataset_SSDD into train/val/test splits and save path/autosplit_*.txt files
...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root, augment, imgsz, cache=False):
        """Initialize YOLO dataset_SSDD with root, augmentation, image size, and cache parameters."""
    ...
    def __getitem__(self, i):
        """Retrieves data items of 'dataset_SSDD' via indices & creates InfiniteDataLoader."""
